        ...
        <div className="relative w-full overflow-hidden pb-10 sm:pb-8">
          <div className="flex animate-marquee hover:pause">
            ...
            {[0, 1, 2, 3].map((setIndex) => (
              <div key={`set-${setIndex}`} className="flex items-end gap-12 md:gap-[72px] shrink-0 pr-12 md:pr-[72px]">
                {slice.items.map((item: any, index) => (
                  <div key={`${setIndex}-${index}`} className="shrink-0 flex flex-col items-center gap-2">
                    <div className="flex items-center justify-center h-10 md:h-14 lg:h-16">
                      {item.logo?.url && (
                        item.case_study_slug ? (
                          <Link href={`/case-study/${item.case_study_slug}`} className="h-full hover:opacity-100 transition-opacity">
                            <PrismicNextImage
                              field={item.logo}
                              className="h-full w-auto max-w-[150px] md:max-w-[180px] lg:max-w-[200px] object-contain grayscale opacity-70 hover:opacity-100 hover:grayscale-0 transition-all"
                              fallbackAlt=""
                            />
                          </Link>
                        ) : (
                    ...
                    {/* Case Study badge - only shown for logos with case study slug */}
                    {item.case_study_slug && (
                      <Link
                        href={`/case-study/${item.case_study_slug}`}
                        className="bg-[#F2F2F2] px-2 py-1 rounded-[2px] flex items-center justify-center hover:bg-[#E8E8E8] transition-colors"
                      >
